...
def main( args):
    ...
    state_dict = torch.load(f"t0.1/epo_{epoch}_bs_{BS}_lr_{lr}_l1_{l1_coeff}_expand_{expand}_ty_sae.pth")
    ...
    with torch.no_grad():
        for x, y in tqdm(loader):
            ...
            _,feat = model(xt, t, **model_kwargs)
            feat = feat.reshape(x.shape[0],256,-1)[:,:,:].sum(dim=1)
            feats.append(feat.cpu())
    feats = torch.cat(feats)
    np.save(f"feats_cond_{expand}.npy",feats.numpy())
    # idx = torch.argsort(feats,descending=True)
    # print(idx[:16])
    # plt.hist(l0n.numpy())
    # plt.title(f"Sparsity of SAE expand ratio {expand}")
    # plt.xlabel("sparsity across training set")
    # plt.ylabel("count")
    # plt.savefig(f"l0n_{expand}.png")
    # print((l0n==0).sum().item())
    # print(l0n.sum().item())
    # print(l0n.mean().item())
